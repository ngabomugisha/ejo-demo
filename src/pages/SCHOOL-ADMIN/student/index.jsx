...
export const Index = (props) => {
    ...
    const onExportClick = () => {
        // const exportdata = props.list.map(data => [{value: data.firstName },{value:data.lastName}])
    ...
    const searchStyle = {
        ...
        border: "2px #1F72C6 solid", fontSize: "100%", marginRight:"10px"
    }
                ...
                class:
                    props.classes.reduce(function (match, check) {
                        if (check._id == i.class) {
                            let find = check.level && check.level.name + " " + (check.combination ? check.combination.name : "") + check.label
                            match = find
                        }
                        return match
                    }, 'a'),
    ...
    const columns = [
        ...
        { headerName: 'ID', field: 'id', hide: true, flex: 1 },
        {
            ...
            cellRendererFramework: (params) => <div style={{ display: "flex" }}>
                <div style={{ color: "#1F72C6", cursor: "pointer", borderRadius: "20px", backgroundColor: "#e8f5ff", marginRight: "10px", textAlign: 'center', paddingLeft: "35px", paddingRight: "35px", verticalAlign: "center", fontWeight: "bold" }} className="edit-btn-class" onClick={() => editRow(params)}>Edit</div>
