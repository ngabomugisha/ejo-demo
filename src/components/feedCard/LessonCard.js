...
function LessonCard(props) {
	...
	const displayCovered = () => {
		...
		unitData &&
			setKnowledge(
				unitData.content.knowledgeAndUnderstanding.filter((val) => {
		...
		unitData &&
			setSkills(
				unitData.content.skills.filter((val) => {
					return val.numberOftimesTaught !== 0;
				})
			);
		...
		unitData &&
			setAttitude(
				unitData.content.attitudesAndValues.filter((val) => {
					return val.numberOftimesTaught !== 0;
				})
			);
	...
	const fetchUintPlan = (unitPlanId) => {
		https
			.get(`/lessons/unit-plans/${unitPlanId}`, {
		...
		<>
			<div className="card-container">
				...
				<div className="card2-body">
					<p className="card2-details">{props.details}</p>
				</div>
				<div className="card2-ft">
					<p className="card2-time">{props.time}</p>
					<p
						className="card2-size"
						onClick={displayExpected}
						style={{ cursor: "pointer" }}
					>
						Expected: {expected}
					</p>
					<p
						onClick={displayCovered}
						className="card2-covered"
						style={{ cursor: "pointer" }}
					>
						Covered: {covered}
					</p>
			...
			<Modal show={show} onHide={handleClose2} animation backdrop size="xl">
				<Modal.Header closeButton>
					<Modal.Title>
						<span style={{ color: "#1F72C6" }}>{title}</span>
					</Modal.Title>
				</Modal.Header>
				<Modal.Body>
